    ...
    public class PaymentManager
    {
        ...
        public async Task<APIResponseResult<getStudentAudits>> AddPaymentAsync(paymentManagerAdd dto)
        {
            if (dto != null)
            {
                ...
                {
                    var addPaymentResult = await _newPaymentService.UpdatePaymentAsync(new PaymentDTO
                    {   Id=payment.Id,
                        MoneyPaid= payment.MoneyPaid+dto.amount,
                        NominatingAuthority= payment.NominatingAuthority,
                        RestOfMoney=payment.RestOfMoney- dto.amount,
                        Notes= dto.notes,
                        State= payment.RestOfMoney - dto.amount == 0? State.FeesPaid: State.FeesUnPaid
                    });
        ...
        public async Task<APIResponseResult<bool>> DeletePaymentAsync(int  auditId)
        {
            if (auditId != null)
            {
                ...
                var updatePaymentResult = await _newPaymentService.UpdatePaymentAsync(new PaymentDTO
                {
                    Id = payment.Id,
                    MoneyPaid = payment.MoneyPaid - paymentAudit.Amount,
                    NominatingAuthority = payment.NominatingAuthority,
                    RestOfMoney = payment.RestOfMoney + paymentAudit.Amount,
                    Notes = payment.Notes,
                    State = payment.RestOfMoney + paymentAudit.Amount == 0 ? State.FeesPaid : State.FeesUnPaid
                });
